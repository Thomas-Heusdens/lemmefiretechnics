          ...
          <div className="space-y-4 md:space-y-8">
            <div className="relative h-80 rounded-3xl overflow-hidden shadow-2xl border border-white/10">
              ...
              <div className="absolute bottom-0 left-0 p-4 md:p-8">
                <div className="inline-flex items-center px-3 py-1 rounded-full bg-red-600/90 text-white text-xs font-bold uppercase tracking-wider mb-2 border border-red-400/30">
                  {formation.category === 'firefighter' ? 'Professionnel' : 'Civil'}
                </div>
                <h1 className="text-xl md:text-3xl font-bold text-white mb-2">{formation.name}</h1>
              </div>
            </div>

            <div className="prose prose-invert max-w-none">
              <h3 className="text-lg md:text-2xl text-center md:text-left font-bold text-white mb-4">À propos de ce cursus</h3>
              <p className="text-gray-300 text-center md:text-left text-base md:text-lg text-base leading-relaxed">{formation.description}</p>
            </div>
            ...
            {brevet && (
              <div className="p-6 bg-gradient-to-br from-gray-900 to-gray-800 border border-red-900/30 rounded-2xl flex items-start gap-5 shadow-lg relative overflow-hidden group">
                ...
                <div>
                  <h4 className="text-red-400 font-bold tracking-wide text-xs mb-1">Brevet Applicable</h4>
                  <div className="text-xl font-bold text-white mb-1">{brevet.name}</div>
                  {brevet.description && (
                    <p className="text-sm text-gray-400">{brevet.description}</p>
                  )}
                </div>
              </div>
            )}
            ...
            <div className="relative w-full">
              <div className="group bg-gray-900 border border-white/10 rounded-2xl md:rounded-3xl overflow-hidden shadow-2xl flex flex-col relative h-full">
                ...
                <div className="flex-1 p-5 md:p-8 flex flex-col relative">
                  ...
                  <div 
                    className={`flex-1 transition-all duration-300 transform ${isAnimating ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'}`}
                  >
                    ...
                    <div className="grid grid-cols-3 gap-2 md:gap-4 mb-6 md:mb-8">
                      <div className="flex flex-col items-center p-2 md:p-4 bg-white/5 rounded-xl border border-white/5">
                        ...
                        <span className="text-[10px] md:text-xs text-gray-500 uppercase font-semibold mb-0.5">Durée</span>
                        <span className="text-white font-bold text-xs md:text-base text-center">{currentLevel.duration}</span>
                      </div>
                      <div className="flex flex-col items-center p-2 md:p-4 bg-white/5 rounded-xl border border-white/5">
                        <Calendar className="w-4 h-4 md:w-5 md:h-5 text-red-500 mb-1 md:mb-2" />
                        <span className="text-[10px] md:text-xs text-gray-500 uppercase font-semibold mb-0.5">Séances</span>
                      ...
                      <div className="flex flex-col items-center p-2 md:p-4 bg-white/5 rounded-xl border border-white/5">
                        ...
                        <span className="text-[10px] md:text-xs text-gray-500 uppercase font-semibold mb-0.5">Module</span>
