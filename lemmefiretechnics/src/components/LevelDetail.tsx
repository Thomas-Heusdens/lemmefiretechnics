    ...
    <div className="fixed inset-0 z-50 bg-gray-950 animate-in fade-in duration-300 overflow-y-auto overflow-x-hidden scrollbar-thin scrollbar-thumb-red-900 scrollbar-track-black">
      ...
      <div className="relative max-w-7xl mx-auto pt-20 md:pt-16 px-5 sm:px-6 lg:px-8 pb-6 md:pb-8">
          ...
          <div className="flex flex-wrap items-center gap-2 md:gap-3">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-red-600/20 border border-red-500/30 rounded-full text-red-400 font-bold uppercase tracking-wider text-[11px] md:text-sm backdrop-blur-sm">
              <ShieldCheck className="w-3 h-3 md:w-4 md:h-4" />
              Formation Certifiante
            </div>
            {level && (
              <div className="inline-flex items-center px-3 py-1.5 bg-white/10 rounded-full border border-white/10 backdrop-blur-md text-white font-semibold text-[11px] md:text-sm">
                Module {level}
              </div>
            )}
          </div>
            ...
            {goals && (
              <div className="mt-6 md:mt-8 pt-6 md:pt-8 border-t border-white/5">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h4 className="text-white font-bold text-base md:text-lg mb-2">But de la formation</h4>
              ...
              {imagesToUse.length > 1 && (
                <>
                  <button 
                      ...
                      className="absolute left-2 top-1/2 -translate-y-1/2 p-2 bg-black/40 hover:bg-red-600 text-white rounded-full backdrop-blur-md border border-white/10 transition-all opacity-100 lg:opacity-0 lg:group-hover:opacity-100 z-30 cursor-pointer"
                  >
                      <ChevronLeft className="w-5 h-5" />
                  </button>
                  <button 
                      onClick={handleNextImage}
                      className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-black/40 hover:bg-red-600 text-white rounded-full backdrop-blur-md border border-white/10 transition-all opacity-100 lg:opacity-0 lg:group-hover:opacity-100 z-30 cursor-pointer"
                  >
                  ...
                  <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-30">
                      {imagesToUse.map((_, idx) => (
                          <div 
                              key={idx} 
                              className={`h-1.5 rounded-full transition-all duration-300 shadow-sm ${idx === selectedImageIndex ? 'w-6 md:w-8 bg-red-600' : 'w-2 bg-white/70'}`}
                          />
                      ))}
                  </div>
                </>
        ...
        <div className="grid lg:grid-cols-3 gap-6 md:gap-8">
          ...
          <div className="relative order-1 lg:order-2">
            <div className="bg-gray-900 border border-red-900/30 rounded-2xl md:rounded-3xl p-6 md:p-8 shadow-2xl shadow-red-900/10 backdrop-blur-xl lg:sticky lg:top-8">
              <h3 className="text-xl md:text-2xl font-bold text-white mb-6 md:mb-8 border-b border-gray-800 pb-4">Détails Pratiques</h3>
              ...
              <div className="space-y-4 md:space-y-6 mb-8">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-white font-bold text-base md:text-lg">{sessionsPerWeek} sess/sem</span>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-white font-bold text-base md:text-lg">Oui</span>
                </div>
              </div>
              ...
              <div className="hidden md:block space-y-4 pt-4 border-t border-gray-800">
                ...
                <p className="text-xs text-center text-gray-500 leading-relaxed">
                  Places limitées. Validation des prérequis requise.
                </p>
              </div>
            </div>
          </div>
